...
%%% Unten: Links, Mitte, Rechts
 \ifoot[]{\vspace{-3pt}\href{http://ansible.cc/}{ansible.cc}}
 \cfoot[]{}
...
modules\footnote{\url{http://ansible.github.com/modules.html}} (called the
"module library") that can be executed directly on remote hosts or through
Playbooks (Fig.~\ref{factslocal}).

\bild{250pt}{art/ansible-facts-local.png}{Ansible copies modules to nodes for execution}{factslocal}

These modules can control system resources, like services, packages, or files
(anything really), or handle executing system commands. The following is a list
of modules in the core library with supported options (\man{} is mandatory,
\opt{} optional). (If you're viewing this in a PDF reader, click on the module
name for its official documentation.)
...
\begin{enumerate}
	...
	Some Linux distributions come with broken init scripts which prevent the \M{service}
	module from completing. If that happens to you, you might want to try using
	the \M{shell} module with the following command:

	\begin{extymeta}
	setsid bash -c '(/etc/init.d/xxxx start) & disown \%\%'
	\end{extymeta}

    \item[5.]

	Do you use \I{Vagrant}? Did you know there's an \ansible{} plugin for
	it\footnote{\url{https://github.com/dsander/vagrant-ansible}}?
...
\subsection*{Resources}
	...
	\begin{itemize}
	\item
		An up-to-date listing of interesting links to \ansible{}
		resources: \href{https://github.com/ansible/ansible-resources}{github.com/ansible/ansible-resources}

	\end{itemize}

\subsection*{Credits}

	This booklet was copy/pasted by Jan-Piet Mens. Its source lives at
	\href{https://github.com/jpmens/ansible-booklet}{github.com/jpmens/ansible-booklet}.
