...
% modules2.py ---> /Users/jpm/Auto/pubgit/ansible/ansible/library/lineinfile
%--- LINEINFILE  ----  from /Users/jpm/Auto/pubgit/ansible/ansible/library/lineinfile ---
...
\begin{xlist}{abcdefghijklmno}
  \item[\man\,\C{name}]
	The file to modify 
		...
		\item[\man\,\C{regexp}]
	The regular expression to look for in the file. For \I{state=present}, the pattern to replace. For \I{state=absent}, the pattern of the line to remove. 
	...
	Whether the line should be there or not. 
		...
		\B{Choices}:\,
		    ...
		    (default \C{present})
		\item[\opt\,\C{line}]
	Required for \I{state=present}. The line to insert/replace into the file. Must match the value given to \C{regexp}. 
		
		\item[\opt\,\C{insertafter}]
	Used with \I{state=present}. If specified, the line will be inserted after the specified regular expression. Two special values are available; \C{BOF} for inserting the line at the beginning of the file, and \C{EOF} for inserting the line at the end of the file. 
